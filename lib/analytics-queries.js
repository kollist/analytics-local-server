...
function computeStats(db, { appSlug = null, platform = null, days = 14 } = {}) {
  ...
  // `days === null` → all time. Every windowed clause just uses a lower bound
  // far enough back to cover the whole table; period-over-period is skipped.
  const allTime = days == null;
  const effDays = allTime ? 36525 : days;         // ~100 years
  ...
  const kpis = windowKpis(db, f, effDays, 0);
  const prev = allTime ? null : windowKpis(db, f, effDays * 2, effDays);
  ...
  return {
    days,
    windowLabel: allTime ? 'all time' : `last ${days} days`,
    bucket: gran,
    kpis, prev,
    funnel, daily,
    checkout, topItems,
