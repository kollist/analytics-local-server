...
function computeStats(db, { appSlug = null, platform = null, days = 14 } = {}) {
  ...
  // `days === null` → "all time", but an unbounded lower bound means every
  // sub-query does a full read of a 300–650 MB index on a RAM-constrained host
  // (minutes), and this is on the page-load path. Cap it at 365 days; a true
  // all-time view belongs on a separate endpoint backed by a rollup, not here.
  // `allTime` still suppresses period-over-period (see `prev` / `perApp`).
  const allTime = days == null;
  const ALL_TIME_CAP_DAYS = 365;
  const effDays = allTime ? ALL_TIME_CAP_DAYS : days;
  const since = `-${effDays} days`;
  const W = `WHERE timestamp >= datetime('now', ?)${f.sql}`;      // current window
  const A = [since, ...f.args];

  // Per-block timing — one line per real computation (cache hits never get here),
  // so we can see which battery in a slow /api/stats is actually the cost.
  const _timing = [];
  let _tPrev = Date.now();
  const mark = (label) => { const t = Date.now(); _timing.push([label, t - _tPrev]); _tPrev = t; };

  // ── KPI strip + period-over-period ──────────────────────────────────────────
  const kpis = windowKpis(db, f, effDays, 0);                             mark('kpis');
  const prev = allTime ? null : windowKpis(db, f, effDays * 2, effDays);  mark('kpisPrev');
  ...
  // ── Time series (zero-filled; bucketed by day / week / month for long spans) ─
  ...
  const firstAgg = dayAgg.reduce((m, r) => (r.day < m ? r.day : m), iso(Date.now()));
  const startMs = allTime
    ? Date.parse(firstAgg + 'T00:00:00Z')
    : Date.now() - (days - 1) * 86400000;
  const spanDaysChart = Math.max(1, Math.round((Date.now() - startMs) / 86400000) + 1);
  const gran = spanDaysChart <= 92 ? 'day' : spanDaysChart <= 731 ? 'week' : 'month';

  const bucketKey = (d) => {
    if (gran === 'month') return d.slice(0, 7) + '-01';
    if (gran === 'week') {
  ...
  const stepBucket = (k) => {
    ...
    if (gran === 'month') x.setUTCMonth(x.getUTCMonth() + 1);
  ...
  const buckets = {};
  const endKey = bucketKey(iso(Date.now()));
  let cur = bucketKey(iso(startMs));
  for (let guard = 0; guard < 2000; guard++) {
    buckets[cur] = { day: cur, bucket: gran, sessions: 0, purchases: 0, revenue: 0 };
    if (cur === endKey) break;
    cur = stepBucket(cur);
  }
  for (const r of dayAgg) {
    const b = buckets[bucketKey(r.day)];
    if (!b) continue;
    b.sessions += r.sessions;
    b.purchases += r.purchaseSessions;
    b.revenue = round2(b.revenue + r.revenue);
  }
  const daily = Object.values(buckets).map(b => ({
    ...b,
  ...
  const perAppCur = db.prepare(`
    ...
    GROUP BY app_slug
  `).all(since, ...pfArgs);
  // Previous-period order counts drive the trend arrows. All time has no prior
  // period (and `days` is null, so `-${days * 2} days` wouldn't be a valid
  // modifier) — skip it and let the client drop the arrow.
  const prevOrders = {};
  if (!allTime) {
  ...
  const perApp = perAppCur.map(r => {
    ...
    return {
      ...
      users: r.users,
      orders: r.orders,
      prevOrders: allTime ? null : (prevOrders[r.app_slug] || 0),
  ...
  const ordersPerDay = {
    ...
    window: allTime ? null : round2(kpis.orders / days),
  ...
  const _totalMs = _timing.reduce((s, [, ms]) => s + ms, 0);
  console.log(
    `[stats] ${appSlug || 'all'}/${platform || 'all'}/${allTime ? 'ALL' : days + 'd'} ` +
    `${_totalMs}ms  ` + _timing.map(([l, ms]) => `${l}=${ms}`).join(' ')
  );

  return {
    days,
    windowLabel: allTime ? `last ${ALL_TIME_CAP_DAYS} days` : `last ${days} days`,
    bucket: gran,
    kpis, prev,
    funnel, daily,
    checkout, topItems,
