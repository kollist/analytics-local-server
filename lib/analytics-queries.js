...
// app / platform filter fragment (independent, both optional)
function filterClause(appSlug, platform) {
  const parts = [], args = [];
...
function computeStats(db, { appSlug = null, platform = null, days = 14 } = {}) {
  const f = filterClause(appSlug, platform);
  ...
  const pfArgs = platform ? [platform] : [];
...
module.exports = { ERROR_EVENT_TYPES, parseProps, summarizeError, redactRow, computeStats, computeErrors };
